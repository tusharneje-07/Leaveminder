...
if ($_SESSION['HODAUTH'] == false) {
    ...
    <body>
        ...
        <header class="d-flex justify-content-center py-3 maincontent">
            <ul class="nav nav-pills">
                <li class="nav-item"><a href="hod-pending-leave.php" class="nav-link">Pending Leave Applications</a></li>
                <li class="nav-item"><a href="hod-granted-leave.php" class="nav-link active">Greanted Leave Applications</a></li>
            </ul>
        </header>
        <!-- 2nd navigation -->
    ...
    while ($row = mysqli_fetch_assoc($query)) {
        ...
        echo '<tr style="border-bottom: 1px solid #000;">
                            ...
                            <button "type="submit" class="btn btn-danger btn-sm" name="grantleave" id="grantleave" value="' . $getdata_leaveid . '">Get Letter</button>
                            </form>
                            </td>
                        </tr>';
        $rown += 1;
    ...
    if (isset($_POST['grantleave'])) {
        echo '<script>
        alert("' . $_POST['grantleave'] . '");
        </script>';

        $query = mysqli_query($conn, "SELECT application_date_time,fromdate,todate,leavetype,reason,leavestatus FROM leavetemp where leaveid='" . $_POST['grantleave'] . "'");
